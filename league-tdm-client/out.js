...
(() => {
  ...
  // ../league-core/node_modules/reflect-metadata/Reflect.js
  var Reflect2;
  (function(Reflect3) {
    (function(factory) {
    ...
    return class extends target {
      constructor(...args) {
        ...
        if (!Reflect.getMetadata(Core.Decorator.commandsInit, target.prototype)) {
          const list = Reflect.getMetadata(Core.Decorator.commands, this) || [];
          for (const { commands, descriptions, descriptor, group, method } of list) {
            ...
            if (group) {
              ...
              mp.events.addCommand(
                group,
                (player, fullText, ...args2) => {
                  const [callableCommand] = args2;
                  ...
                  callback.apply(this, [player, fullText, descriptions[commandIndex], ...args2]);
                }
              );
            } else {
              commands.forEach((name, index) => {
                ...
                mp.events.addCommand(
                  name,
                  (player, fullText, ...args2) => callback.apply(this, [player, fullText, descriptions[index], ...args2])
                );
              });
            }
          }
  ...
  var client = (target) => {
    return class extends target {
      constructor(...args) {
        super(...args);
        if (!Reflect.getMetadata(Core.Decorator.commandsInit, target.prototype)) {
          const list = Reflect.getMetadata(Core.Decorator.commands, target.prototype) || [];
          const indexByCommand = {};
          ...
          mp.events.add("playerCommand", (input) => {
            try {
              const args2 = input.split(" ");
              const [groupName, ...commandArgs] = args2;
              ...
              if (commandInfo.group) {
                ...
                if (!groupCommandInfo) {
                  (descriptions || []).forEach((text) => mp.gui.chat.push(text));
                  return;
                } else {
  ...
  var commandable = env === Core.Enviroment.client ? client : server;

  // ../league-core/src/decorators/event.ts
  var event = (eventName) => {
  ...
  // ../league-core/src/decorators/eventable.ts
  var eventable = (target) => {
    return class extends target {
      constructor(...args) {
        super(...args);
        if (!Reflect.getMetadata(Core.Decorator.eventsInit, target.prototype)) {
          const list = Reflect.getMetadata(Core.Decorator.events, target.prototype) || [];
          for (const { events, descriptor, method } of list) {
            ...
            events.forEach((eventName) => {
              ...
              mp.events.add(eventName, (...args2) => callback.apply(this, args2));
            });
          }
